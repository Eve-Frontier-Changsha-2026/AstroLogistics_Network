...
import { useMyTransportOrders } from './useTransportOrders';
import { TYPE } from '../config/contracts';

describe('useMyTransportOrders', () => {
  beforeEach(() => vi.clearAllMocks());

  it('queries TransportOrder type', async () => {
    mockClient.listOwnedObjects.mockResolvedValueOnce(mockListOwnedObjectsResponse([]));
    const { result } = renderHook(() => useMyTransportOrders(), { wrapper: TestProvider });
